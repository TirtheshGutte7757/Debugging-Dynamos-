...
const FaceScan: React.FC<FaceScanProps> = ({ onSuccess, onClose, title, mode, userEmail }) => {
  ...
  const handleCompareAndVerify = useCallback(async () => {
    ...
    if (!liveImageDataUrl || !registeredImageUrl) {
    ...
    try {
        ...
        // Check if registeredImageUrl is a data URL or a regular URL that needs fetching
        const registeredImageBase64 = registeredImageUrl.startsWith('data:')
            ? getBase64FromDataUrl(registeredImageUrl)
            : await fetchAndConvertToBase64(registeredImageUrl);
  ...
  }, [mode, onSuccess, stopCamera, registeredImageUrl]);
